...
define('PHP_FIREWALL_ADMIN_MAIL', getenv('SERVER_ADMIN') );
define('PHP_FIREWALL_PUSH_MAIL', true );
define('PHP_FIREWALL_LOG_FILE', $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST']."/framework/".getenv('FIREWALL_LOG'));
...
if (@ PHP_FIREWALL_ACTIVATION === true ) {
	...
	FUNCTION PHP_FIREWALL_LOGS( $type ) {
		...
		$f = fopen( PHP_FIREWALL_LOG_FILE, 'a');
        ...
        $msg = date('Y-m-d H:i:s')."\t  USER ".gethostbyaddr(PHP_FIREWALL_GET_IP)." with ".PHP_FIREWALL_GET_REFERER." and IP ".PHP_FIREWALL_GET_IP." requested url ".$_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'].PHP_FIREWALL_REQUEST_URI." with firewall response as ".$type."\n\n";
		fputs($f, $msg);
		fclose($f);
